...
/*Strategy:
1) Find courses using a keyword search of descriptions in prod.discovery.course_metadata_course and EMSI skills.
--LIMITATIONS:
...
with emsi_course_skills_map   as (
   ...
   --select count(distinct course_key) from emsi_course_skills_map;
   ,
    /*narrow to set of courses with at least one active courserun in 2022 AND in the B2B subscription catalog*/
    relevant_courses          as (
        select distinct a.course_key, a.course_number_id as course_id
...
/*Add description information to selected courses table*/
   ...
   , selected_courses         as (
    ...
    order by rc.course_key desc
)

/*Join course_id to verfied users in dim_enrollments using course_key.
Note: I selected 2019 to the end of 2021 for enrollments.
This bakes in a few months for people to earn certificates and avoids pre-2019 data quality issues*/
   , enrollments              as
    (
        ...
        from prod.core.dim_enrollments          de
            ...
            /*Join selected course_ids to remove unselected courses out of dim_enrollment */
   ...
   , course_engagement        as
    (
        ...
        from prod.core.fact_enrollment_engagement ee
             ...
             join      prod.core.dim_enrollments  pc
                       on ee.user_id = pc.user_id and ee.courserun_key = pc.courserun_key
        where pc.first_downloadable_certificate_date is not null
    )
/* Calculate summary statistics for each course_id including one standard deviation from the average.*/
...
--Note:
--1)Explore why 79 courses had all nulls for metrics
--2)Removed Grid Courses- 5675, 4237, 5676
